...
void Overlay::Update(MemSnapshot& memSnapshot) {
  ...
  duration<float, std::milli> timeDuration =
      high_resolution_clock::now() - timeBefore;
...
ID3D11Device* Overlay::GetDxDevice() { return dxDevice; }
...
extern IMGUI_IMPL_API LRESULT ImGui_ImplWin32_WndProcHandler(HWND hWnd,
                                                             ...
                                                             WPARAM wParam,
                                                             LPARAM lParam);
LRESULT WINAPI Overlay::WndProc(HWND hWnd, UINT msg, WPARAM wParam,
                                LPARAM lParam) {
  ...
  switch (msg) {
    case WM_SIZE:
      if (dxDevice != NULL && wParam != SIZE_MINIMIZED) {
        CleanupRenderTarget();
        dxSwapChain->ResizeBuffers(0, (UINT)LOWORD(lParam),
