...
// Interface for creating UIs with imgui.
class ImguiInterface {
 public:
  ...
  void LabelTextColored(const char* label, const char* text,
                        const ImVec4& color) {
  ...
  ImVec4 ColorPicker(const char* label, const ImVec4& color) {
    ImGui::ColorPicker4(label, (float*)&color);
    return color;
  }

  int DragInt(const char* text, int i, int step = 1, int minVal = 0,
              int maxVal = 0) {
    ImGui::DragInt(text, &i, (float)step, minVal, maxVal);
    return i;
  }

  float DragFloat(const char* text, float i, float step = 1, float minVal = 0,
                  float maxVal = 0) {
  ...
  int ListBox(const char* label, const std::vector<std::string>& items,
              int chosen) {
    static std::vector<const char*> buffer;

    buffer.clear();
    int size = items.size();
    for (int i = 0; i < size; ++i) buffer.push_back(items[i].c_str());
