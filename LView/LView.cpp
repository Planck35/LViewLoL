...
void MainLoop(Overlay& overlay, LeagueMemoryReader& reader) {
  ...
  while (true) {
    ...
    if (overlay.IsVisible()) {
      // One some systems the ingame cursor is replaced with the default Windows
      // cursor With the WS_EX_TRANSPARENT window flag enabled the cursor is as
      // expected but the user cannot control the overlay
    ...
    try {
      ...
      // Try to find the league process and get its information necessary for
      // reading
      if (rehook) {
        ...
        memSnapshot = MemSnapshot();
        printf(
            "[i] Found league process. The UI will appear when the game "
            "stars.\n");
      } else {
...
/// Authentication using AWS. Calls a lambda from AWS that will do the
authentication. bool Authenticate() {
        ...
        if (region.empty() || name.empty() || accessKey.empty() ||
secretKey.empty()) { printf("[!] auth:: config fields are missing"); return
false;
        }
...
Aws::MakeShared<Aws::StringStream>("FunctionTest"); Aws::Utils::Json::JsonValue
json; json.WithString("operation", "auth"); json.WithString("name",
name.c_str()); json.WithString("secret_key", secretKey.c_str());
        json.WithString("access_key", accessKey.c_str());
        json.WithString("hwid", hwid.c_str());
        *payload << json.View().WriteReadable();

        auto client = Aws::MakeShared<Aws::Lambda::LambdaClient>("alloc_tag",
credentials, config); Aws::Lambda::Model::InvokeRequest invokeRequest;
