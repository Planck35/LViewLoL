...
void GameObject::LoadFromMem(DWORD base, HANDLE hProcess, bool deepLoad) {
  ...
  // Store previous position once N milliseconds to avoid the case when position
  // == previousPosition but the object is moving
  std::chrono::duration<float, std::milli> timeDuration =
      high_resolution_clock::now() - timeSinceLastPreviousPosition;
...
Spell* GameObject::GetSummonerSpell(SummonerSpellType type) {
  if (D.summonerSpellType == type) return &D;
  if (F.summonerSpellType == type) return &F;
  return nullptr;
}
