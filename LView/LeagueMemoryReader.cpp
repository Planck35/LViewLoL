...
///		This method reads the game objects from memory. It reads the tree
///structure of a std::map<int, GameObject*>
...
void LeagueMemoryReader::ReadObjects(MemSnapshot& ms) {
  ...
  while (reads < maxObjects && nodesToVisit.size() > 0) {
    ...
    if (netId - (unsigned int)0x40000000 > 0x100000) continue;
  ...
  for (int i = 0; i < nrObj; ++i) {
    ...
    Mem::Read(hProcess, pointerArray[i] + Offsets::ObjNetworkID, &netId,
              sizeof(int));
    if (blacklistedObjects.find(netId) != blacklistedObjects.end()) continue;
    ...
    if (obj->networkId != 0) {
      ...
      if (obj->name.size() <= 2 || blacklistedObjectNames.find(obj->name) !=
...
void LeagueMemoryReader::FindPlayerChampion(MemSnapshot& snapshot) {
  ...
  else  // If we can't find the local player either the offset is wrong or we
        // are watching a replay
    snapshot.player =
