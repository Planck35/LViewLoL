...
std::string GetPyError() {
  ...
  PyObject* errExcLineNum =
      PyObject_Str(PyObject_GetAttrString(tb, "tb_lineno"));
...
bool Script::LoadInfo() {
  ...
  if (dictAttr == NULL) {
    loadError =
        std::string("No `lview_script_info` dictionary found in script");
    return false;
  }
  ...
  try {
    ...
    loadError = std::string(
        "Script info dictionary contains wrong values types or missing values");
    return false;
  }
...
void Script::Load(const char* file) {
  ...
  if (NULL == moduleObj) {
    ...
    if (LoadInfo() && LoadFunc(&updateFunc, "lview_update") &&
...
void Script::ExecUpdate(const PyGame& state, const PyImguiInterface& ui) {
  try {
    if (NULL != updateFunc) {
      high_resolution_clock::time_point beforeUpdate =
