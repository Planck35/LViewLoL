...
bool Input::WasKeyPressed(HKey key) {
  ...
  static high_resolution_clock::time_point lastTimePressed[300] = {
  ...
  if (pressed[virtualKey]) {
    if (timeDiff.count() > 200) pressed[virtualKey] = false;
    return false;
  }
...
void Input::MoveCursorTo(float x, float y) {
  ...
  // Sometimes this fails idk why the fuck but calling the function two times
  // seems to solve it
  SendInput(1, &input, sizeof(INPUT));
  SendInput(1, &input, sizeof(INPUT));
}
